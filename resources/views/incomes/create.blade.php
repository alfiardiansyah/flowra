        ...
        <form action="{{ route('incomes.store') }}" method="POST" class="space-y-6">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                    @php
                        $displayCats = $categories->count() > 0 ? $categories : collect([
                            (object)['id' => '', 'name' => 'Gaji', 'icon' => 'sunflower', 'color' => '#87A96B'],
                            (object)['id' => '', 'name' => 'Bonus', 'icon' => 'cherry-blossom', 'color' => '#FF7A5C'],
                            (object)['id' => '', 'name' => 'Investasi', 'icon' => 'oak-tree', 'color' => '#6B8E23'],
                            (object)['id' => '', 'name' => 'Freelance', 'icon' => 'wildflower', 'color' => '#9BCF53'],
                            (object)['id' => '', 'name' => 'Lainnya', 'icon' => 'bouquet', 'color' => '#5DADE2']
                        ]);
                    @endphp
                    @foreach($displayCats as $cat)
                        <label class="relative cursor-pointer">
                            ...
                            <div class="flex flex-col items-center gap-2 p-3.5 rounded-xl border-2 border-sage-200 peer-checked:border-leaf-500 peer-checked:bg-mint-50/60 hover:border-sage-400 transition-all duration-200 text-center">
                                <x-icon :name="$cat->icon ?? 'sunflower'" class="w-7 h-7" style="color: {{ $cat->color ?? '#87A96B' }}" />
                                <span class="text-xs font-semibold text-earth-700">{{ $cat->name }}</span>
                            </div>
                        </label>
                    @endforeach
                </div>
                <input type="hidden" name="kategori" id="hidden_kategori" value="{{ old('kategori', $displayCats->first()->name ?? 'Gaji') }}">
                @error('category_id')
                    <p class="mt-1 text-sm text-coral-600">{{ $message }}</p>
                @enderror
            </div>

            <!-- Rekening / Dompet Penerima -->
            <div class="form-section">
                <label class="form-section-title">
                    <x-icon name="cash-leaf" class="w-5 h-5" />
                    Rekening / Dompet Masuk
                </label>
            ...
            <div class="form-section">
                ...
                <input type="text" name="keterangan" id="keterangan" 
                       ...
                       placeholder="Contoh: Gaji Bulanan, Bonus Proyek, dll">
                @error('keterangan')
                    <p class="mt-1 text-sm text-coral-600">{{ $message }}</p>
                @enderror
            </div>
